<x-app-layout>
    ...
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-3">
            <div>
                <h2 class="fw-bold fs-3 mb-1">{{ isset($student) ? 'Edit Student' : 'Create New Student' }}</h2>
                <p class="text-muted mb-0 small">
                    {{ isset($student) ? 'Update student details' : 'Add a new student to the system' }}
                </p>
            </div>
            <div>
                <x-back-button :href="route('students.index')" class="btn btn-outline-secondary btn-sm">
    ...
    <x-slot name="breadcrumbs">
        ...
        <li class="breadcrumb-item"><a href="{{ route('students.index') }}" class="text-decoration-none">Students</a>
        </li>
        <li class="breadcrumb-item active" aria-current="page">{{ isset($student) ? 'Edit' : 'Create' }}</li>
    </x-slot>
                ...
                <form action="{{ isset($student) ? route('students.update', $student) : route('students.store') }}"
                    ...
                    <div class="col-md-6">
                        <label for="password" class="form-label fw-semibold">
                            {{ isset($student) ? 'New Password' : 'Password' }}
                        ...
                        <div class="input-group">
                            <input type="password" name="password" id="password"
                                ...
                                placeholder="{{ isset($student) ? 'Leave blank to keep current password' : 'Minimum 8 characters' }}">
                    ...
                    <!-- Class -->
                    <div class="col-md-6">
                        <label for="school_class_id" class="form-label fw-semibold">Class <span
                                class="text-danger">*</span></label>
                        ...
                        <select name="gender" id="gender" class="form-select @error('gender') is-invalid @enderror">
                            ...
                            @foreach (['male', 'female', 'other', 'unspecified'] as $genderOption)
                                <option value="{{ $genderOption }}"
                                    {{ old('gender', $student->user->gender ?? '') === $genderOption ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="dob" id="dob"
                            ...
                            value="{{ old('dob', isset($student) && $student->user->dob ? $student->user->dob->format('Y-m-d') : '') }}">
                        @error('dob')
                            <x-error-msg>{{ $message }}</x-error-msg>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="blood_group" id="blood_group"
                            ...
                            @foreach (['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'] as $bg)
                                <option value="{{ $bg }}"
                                    {{ old('blood_group', $student->user->blood_group ?? '') === $bg ? 'selected' : '' }}>
                                    {{ $bg }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" id="status"
                            ...
                            @foreach (consthelper('User::$statuses') as $id => $status)
                                <option value="{{ $id }}"
                                    {{ old('status', $student->user->status ?? 1) == $id ? 'selected' : '' }}>
                                    {{ $status }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        @if (isset($student->user) && $student->user->profileImage)
                            <div class="mb-2 text-center">
                                <img src="{{ Storage::url($student->user->profileImage->path) }}" alt="Profile Image"
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="address" id="address" rows="3" class="form-control @error('address') is-invalid @enderror">{{ old('address', $student->user->address ?? '') }}</textarea>
                        @error('address')
                            <x-error-msg>{{ $message }}</x-error-msg>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 d-flex justify-content-end gap-2 mt-4">
                        ...
                        <button type="submit" class="btn btn-primary px-4"><i class="bi bi-save me-1"></i>
                            {{ isset($student) ? 'Update Student' : 'Create Student' }}</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                $(function() {
                    ...
                    try {
                        entries = {!! json_encode(old('additional_information', $student->additional_information ?? '{}')) !!};
